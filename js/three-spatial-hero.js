/* ============================================================
   LIFE ADMIN COPILOT — Upgraded 3D Holographic Spatial Console
   Warm Antique Gold & Dusty Teal Lighting, Physical 3D Frosted Glass Vault, Dual Laser Rings, Floating Spatial Tokens.
   ============================================================ */
...
class SpatialHero3D {
  ...
  init() {
    ...
    // 3. Physical Frosted Glass Document Vault (The Central 3D Object)
    const docGeo = new THREE.BoxGeometry(2.3, 3.1, 0.06);
    ...
    // Document Gold Wireframe Edge Glow
    ...
    const laserGeo = new THREE.PlaneGeometry(2.8, 0.08);
    ...
    // 5. Dual Orbiting Holographic Rings (Gold + Dusty Teal)
    ...
    // 6. Floating Extracted Data Token Badges
    const tokenData = [
      { text: "₹500 DUE TODAY", color: 0xE8C872, x: 2.2, y: 0.9, z: 0.6 },
      { text: "IIT MADRAS", color: 0x7CAABD, x: -2.3, y: 1.0, z: 0.8 },
      { text: "AUG 14 · 6:00 PM", color: 0xD4715E, x: -2.1, y: -0.9, z: 0.7 },
      { text: "PROVENANCE 98.4%", color: 0xE8C872, x: 2.2, y: -0.9, z: 0.9 }
    ];

    tokenData.forEach((token, idx) => {
      const plateGeo = new THREE.BoxGeometry(1.5, 0.45, 0.04);
    ...
    // 8. Warm Editorial Lighting
    const ambient = new THREE.AmbientLight(0xFFFFFF, 1.2);
    this.scene.add(ambient);

    const goldLight = new THREE.PointLight(0xE8C872, 3.5, 12);
    goldLight.position.set(3, 3, 4);
    this.scene.add(goldLight);

    const tealLight = new THREE.PointLight(0x7CAABD, 2.8, 10);
  ...
  animate() {
    ...
    if (this.laserBeam) {
      this.laserBeam.position.y = Math.sin(time * 2.2) * 1.35;
    }

    // Orbiting Laser Rings
    if (this.laserRingGold) {
      this.laserRingGold.rotation.z = time * 0.4;
      this.laserRingGold.rotation.x = Math.sin(time * 0.5) * 0.2;
    }

    if (this.laserRingTeal) {
      this.laserRingTeal.rotation.z = -time * 0.3;
      this.laserRingTeal.rotation.y = Math.cos(time * 0.6) * 0.2;
    }

    // Floating Spatial Data Tokens
